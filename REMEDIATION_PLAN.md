...
| 1.5 | §3.3 | Replace the hardcoded `return True` in `supports_24bit_color()` with an actual check (`os.environ.get('COLORTERM') in ('truecolor', '24bit')`, falling back to a `curses.tigetnum('colors')` / terminfo check). Keep the function's existing call sites unchanged — only the detection logic changes. | `termbook.py:3913-3917` | manual: run in a plain `TERM=xterm` (no COLORTERM) session and confirm it no longer forces 24-bit rendering; run in a real truecolor terminal and confirm it still does | 1-2 hr |
...
**Verification for the whole phase:** flip the Phase-0 `xfail` markers to real assertions; the full `tests/test_language_heuristics.py` (both the target cases and the non-regression corpus) must pass green. Additionally, manually re-run the audit's exact repro snippets (TS class, JS class, 2-line Python function, "renew"/"static" prose sentence) and confirm corrected output.

**Exit criteria:** no known-good case from the audit or Phase 0 corpus misclassifies; the three keyword lists are reduced to one.
