...
**Recommendation (revised after empirical check during Phase 2 implementation):** this section originally recommended making Pygments' `guess_lexer` the primary path and using the hand-written heuristics only to override its known weak spots. Tested directly against the exact repro snippets before writing any fix, `guess_lexer` alone did *worse* than the current heuristic chain: it picked essentially arbitrary lexers from its full ~500-lexer candidate pool for short/ambiguous snippets (the TypeScript class above came back "GDScript", the C ternary snippet came back "Tera Term macro", the 2-line Python function and the SQL query both came back "scdoc"). It only got the unambiguous cases right (XML with a full prolog; the "renew"/"static" prose text, correctly as "Text only"). So promoting `guess_lexer` to primary is not the fix — the heuristic-first structure should stay, but its ordering, word-boundary handling, and per-language trigger lists need to be fixed directly (done in Phase 2; see REMEDIATION_PLAN.md). For `_looks_like_code`, replace the sprawling weighted score with a small number of high-precision signals (e.g., presence of the source book's own "Listing"/"Code" caption, a language shebang/marker, or `<pre>`/`<code>` semantics carried from the EPUB's HTML, which the parser already has access to and would be far more reliable than reconstructing intent from plain text after the fact — see §2.4).
...
| 3.3 | `supports_24bit_color()` unconditionally returned `True` ("Always return True to assume truecolor support as requested") regardless of actual `$COLORTERM`/`$TERM` capability. **Correction (found during Phase 1 remediation):** the function had zero call sites anywhere in the file — it was dead code, not something actually gating rendering decisions. Removed rather than fixed in place (REMEDIATION_PLAN.md §1.5); noted here so the original severity assessment isn't taken at face value. | `termbook.py:3809-3812` (pre-removal) | Low (was dead code, not a live bug) |
